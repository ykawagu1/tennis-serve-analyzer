  ...
  const formatAIResponse = (text) => {
    ...
    lines.forEach((line, index) => {
      ...
      if (trimmedLine === '') {
        ...
        elements.push(
          <div key={`check-${index}`} className="flex items-center space-x-3 mb-2 bg-gray-50 p-3 rounded">
        ...
        // 番号付きリスト
        elements.push(
          <div key={`num-${index}`} className="flex items-start mb-2">
            <span className="text-green-600 mr-3 mt-1 font-semibold">{trimmedLine.match(/^\d+\./)[0]}</span>
        ...
        // 見出し風のテキスト
        elements.push(
          <h4 key={`heading-${index}`} className="text-lg font-semibold text-purple-700 mb-2 mt-4">
            {trimmedLine}
          </h4>
        );
      } else {
        // その他のテキスト
        elements.push(
          <p key={`text-${index}`} className="text-gray-700 leading-relaxed mb-3">
            {trimmedLine}
          </p>
        );
      }
