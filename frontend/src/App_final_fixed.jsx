...
function App() {
  ...
  const formatAIResponse = (text) => {
    ...
    const elements = [];
    let currentList = [];
    let listType = null; // 'ul' or 'ol' or 'checklist'
    
    const flushList = () => {
      if (currentList.length > 0) {
        if (listType === 'checklist') {
          elements.push(
            <div key={`checklist-${elements.length}`} className="bg-gray-50 rounded-lg p-4 mb-4">
          ...
          elements.push(
            <ul key={`ul-${elements.length}`} className="list-none space-y-2 mb-4 ml-4">
              {currentList.map((item, index) => (
          ...
          elements.push(
            <ol key={`ol-${elements.length}`} className="list-none space-y-2 mb-4 ml-4">
              {currentList.map((item, index) => (
                <li key={index} className="flex items-start">
                  <span className="text-green-600 mr-3 mt-1 font-semibold">{index + 1}.</span>
                  <span className="text-gray-700 leading-relaxed">{item}</span>
                </li>
              ))}
            </ol>
    ...
    lines.forEach((line, index) => {
      ...
      // 通常のリスト項目の処理
        ...
        // 太字の処理
        const formattedContent = content.replace(/\*\*(.*?)\*\*/g, '<strong class="font-semibold text-gray-800">$1</strong>');
      ...
      // 太字見出しの処理
        ...
        flushList();
        elements.push(
          <h5 key={`h5-${index}`} className="text-lg font-semibold text-purple-700 mb-2 mt-4">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-lg shadow-lg p-8">
                <h3 className="text-xl font-bold text-gray-800 mb-6 flex items-center">
                  ...
                  {analysisResult.advice?.enhanced ? 'GPT-4o詳細解析レポート' : 'AIアドバイス'}
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                      <div className="flex items-start">
                        ...
                        <div>
                          ...
                          <p className="text-yellow-700 text-sm mt-1">基本的なアドバイスを表示しています。</p>
                        </div>
                      </div>
                    </div>
                  )}

                  {/* GPT-4oの詳細アドバイス（改良されたマークダウン処理） */}
