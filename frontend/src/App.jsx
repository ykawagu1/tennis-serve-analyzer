...
function App() {
  ...
  // ←★ここにコピペ！
    if (!isPremium && usageCount >= FREE_LIMIT) setShowUpgrade(true);
    else setShowUpgrade(false);
  }, [usageCount, isPremium]);
  useEffect(() => {
    console.log('🔍 状態変化検出:');
  ...
  if (usageCount >= USAGE_LIMIT) {
    ...
    try {
      ...
      formData.append('use_chatgpt', useChatGPT && isPremium ? 'true' : 'false');
      ...
      if (useChatGPT && apiKey && isPremium) {
  ...
  return (
    ...
    <div style={{ textAlign: "center", color: "#888", fontSize: 14, marginBottom: 4 }}>
      {isPremium
      ...
      {/* --- 以降はあなたの既存UI（upload、解析、結果、アドバイスなど）をそのまま --- */}
      {/* ...省略... */}
      {/* 解析ボタンだけ下記のように分岐して下さい */}
      <div className="action-buttons">
        ...
        <button
          ...
          disabled={!selectedFile || isAnalyzing || (!isPremium && usageCount >= FREE_LIMIT)}
